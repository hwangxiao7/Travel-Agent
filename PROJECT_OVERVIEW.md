...
> AI 驱动的北美「说走就走」旅行规划（iOS 主产品 + Web Beta + FastAPI Agent）。  
> 输入出发地 + 约束，或一句自然语言（如「想附近找个可以冲浪的地方」），生成有事实依据的逐日行程。  
> 本文档描述当前架构与 RAG 设计，可用作项目说明 / 面试素材。  
> **面试资料（一站式）** → [docs/面试资料.md](./docs/面试资料.md)  
> **面试背稿（栈 / Agent / Q&A）** → [docs/面试技术总结.md](./docs/面试技术总结.md)（→ 面试资料）  
> **现状 / 功能 / AI Agent 工程经验** → [docs/项目现状与AI-Agent工程实践.md](./docs/项目现状与AI-Agent工程实践.md)  
