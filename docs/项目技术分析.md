...
| 客户端 | **iOS 主产品**（SwiftUI）；**Web Beta**（同构 UI，便于外部分享评测） |
...
| LLM | OpenAI 兼容协议（当前 **ByteDance Ark**；可切 Ollama / DeepSeek / 云厂商） |
...
Vite `host: true` + `allowedHosts: true`，便于 Cloudflare Tunnel 外部分享；**不依赖 TestFlight**，用链接/二维码做廉价外测。
...
| `OPENAI_BASE_URL` + `OPENAI_MODEL` + `OPENAI_API_KEY` | 兼容 Ark / Ollama / DeepSeek |
| `EMBEDDING_BACKEND` | `api` \| `local` |
| `LOCAL_EMBED_MODEL` | 本地向量模型 |
| `RERANK_*` | 可选 cross-encoder |

当前运行时：**Ark 聊天 + 本地 embedding**。业务代码只依赖 OpenAI 兼容 `chat.completions`，换 endpoint 不改编排逻辑。
...
| 外测 Web | Vite + Cloudflare Tunnel（笔记本需在线） |
...
| Ark 不可用 | 切 DeepSeek / 其他 OpenAI 兼容端（配置级） |
...
*文档对应仓库当前实现：FastAPI agents/services 分层、iOS + Web 双模式、Ark 聊天 + 本地 embedding。*
